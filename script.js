// Configuration PDF.js avec fichier local
pdfjsLib.GlobalWorkerOptions.workerSrc =
  "libs/pdf.js/pdf.worker.min.js";
...
// Mode offline - Supabase désactivé
document.addEventListener("DOMContentLoaded", function () {
  console.log("Application en mode offline - statistiques désactivées");
});

function checkConsentAndDownload() {
  // Mode offline - téléchargement direct sans collecte de données
...
function prepareDataForSupabase() {
  ...
  Object.keys(cotisationsData).forEach((year) => {
    ...
    const analysisYearData = analysisData.totalByYear[year] || {
      mutuelle: 0,
      secu: 0,
    };
...
async function analyzeDocument(text) {
  ...
  console.log(
    "Analyse du texte PDF (premiers 2000 caractères):",
  ...
  for (const pattern of namePatterns) {
    ...
    if (matches && matches[0]) {
      data.beneficiary = matches[0]
        .replace(/^(Nom|Bénéficiaire|Assuré)[:\s]+/i, "")
        .trim();
  ...
  const tablePatterns = [
    // Harmonie Mutuelle format
    {
      ...
      pattern:
        /(\d{2}\/\d{2}\/\d{4})\s+([A-Za-zÀ-ÿ\s]+?)\s+([\d\s,]+)\s*€\s+([\d\s,]+)\s*€\s+([\d\s,]+)\s*€\s+([\d\s,]+)\s*€\s+([\d\s,]+)\s*€/g,
      fields: [
        "date",
        "libelle",
        "montant_total",
    ...
    // MGEN format
    {
      name: "MGEN",
      pattern:
        /(\d+)\s+([A-Z][a-z]+\s+[A-Z][a-z]+)\s+([A-Za-z\s]+)\s+(\d{2}\/\d{2}\/\d{4})\s+(\d+)\s*€\s+(\d+)\s*€\s+(\d+)\s*€\s+(\d+)\s*€/g,
      fields: [
        "ref",
        "nom",
        "acte",
    ...
    // Generic format 1
    {
      name: "Generic 1",
      pattern:
        /(\d{2}\/\d{2}\/\d{4})\s+([A-Za-zÀ-ÿ\s]+?)\s+([\d,]+)\s*€\s+([\d,]+)\s*€/g,
    ...
    // Generic format 2 (with more columns)
    {
      name: "Generic 2",
      pattern:
        /(\d{2}\/\d{2}\/\d{4})\s+([A-Za-zÀ-ÿ\s]+?)\s+[A-Z\s]+\s+\d+\s+([\d,]+)\s*€\s+([\d,]+)\s*€/g,
      fields: ["date", "libelle", "secu", "mutuelle"],
    },
  ...
  for (const tableFormat of tablePatterns) {
    ...
    while (
      (match = tableFormat.pattern.exec(text)) !== null &&
      matchCount < 1000
    ) {
      matchCount++;
      foundData = true;
      ...
      if (tableFormat.name === "Minimal Format") {
        ...
        console.log(
          `Narrative: ${libelle} - SS: ${secuAmount}€ - Mutuelle: ${mutuelleAmount}€`
        );
        ...
        console.log(
          `Multi-Beneficiary: ${libelle} - Remb total: ${rembTotal}€`
        );
        ...
        console.log(
          `Malakoff: ${libelle} - SS: ${secuAmount}€ - Mutuelle: ${mutuelleAmount}€`
        );
        ...
        console.log(
          `Allianz: ${libelle} - SS: ${secuAmount}€ - Mutuelle: ${mutuelleAmount}€`
        );
        ...
        tableFormat.fields.includes("secu") &&
        tableFormat.fields.includes("mutuelle")
      ) {
        dateStr = match[1];
        libelle = match[2].trim();
    ...
    if (matchCount > 0) {
      console.log(
        `Format ${tableFormat.name} utilisé avec succès: ${matchCount} entrées trouvées`
      );
      break;
    }
  }
  ...
  console.log(
    "Total remboursements trouvés:",
    data.reimbursements.length + data.pharmacyPayments.length
  );
...
function displayDetails(data) {
  ...
  allItems.forEach((item) => {
    rows += `
                    <tr>
                        <td>${item.date}</td>
                        <td class="amount-cell positive">${item.secuAmount.toFixed(
                          2
                        )}€</td>
                        <td class="amount-cell positive">${item.mutuelleAmount.toFixed(
                          2
                        )}€</td>
                        <td class="amount-cell positive"><strong>${item.total.toFixed(
                          2
                        )}€</strong></td>
                    </tr>
                `;
  ...
  tbody.innerHTML =
    rows ||
    '<tr><td colspan="4" style="text-align: center; color: #718096;">Aucun remboursement trouvé</td></tr>';
}
                ...
                <div class="year-section">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="employeur-${year}" 
                                   ...
                                   value="${
                                     cotisationsData[year]
                                       ? cotisationsData[year].employeurMensuel
                                       : ""
                                   }"
                                   onchange="calculateCotisations('${year}')">
                        </div>
                    ...
                    <div class="calculations" id="calc-${year}">
                        <div class="calc-row">
                            ...
                            <span id="total-cotisation-${year}">${
    cotisationsData[year]
      ? cotisationsData[year].totalCotisationAnnuelle.toFixed(2) + "€"
      : "0.00€"
  }</span>
                        </div>
                        <div class="calc-row">
                            <span>Cotisation annuelle (vous) :</span>
                            <span id="votre-cotisation-${year}">${
    cotisationsData[year]
      ? cotisationsData[year].votreCotisationAnnuelle.toFixed(2) + "€"
      : "0.00€"
  }</span>
                        </div>
                        <div class="calc-row">
                            <span>Contribution employeur annuelle :</span>
                            <span id="contrib-employeur-${year}">${
    cotisationsData[year]
      ? cotisationsData[year].employeurAnnuel.toFixed(2) + "€"
      : "0.00€"
  }</span>
                        </div>
                        <div class="calc-row">
                            <span>Total remboursements reçus :</span>
                            <span id="rembours-recu-${year}">${
    data.totalByYear[year] ? data.totalByYear[year].mutuelle.toFixed(2) : "0.00"
  }€</span>
                        </div>
                        <div class="calc-row">
                            <span>Bilan :</span>
                            <span id="deficit-${year}" style="color: ${
    cotisationsData[year] && cotisationsData[year].deficit > 0
      ? "#38a169"
      : "#e53e3e"
  }">
                                ${
                                  cotisationsData[year]
                                    ? cotisationsData[year].deficit.toFixed(2) +
                                      "€"
                                    : "0.00€"
                                }
                            </span>
...
function calculateCotisations(year) {
  ...
  const remboursementsRecus = analysisData.totalByYear[year]
    ? analysisData.totalByYear[year].mutuelle
    : 0;
  ...
  document.getElementById(`votre-cotisation-${year}`).textContent =
    votreCotisationAnnuelle.toFixed(2) + "€";
  document.getElementById(`contrib-employeur-${year}`).textContent =
    employeurAnnuel.toFixed(2) + "€";
  document.getElementById(`rembours-recu-${year}`).textContent =
    remboursementsRecus.toFixed(2) + "€";
...
function displayAnalytics(data) {
  ...
  const avgReimbursement =
    totalItems > 0 ? data.totalReimbursements / totalItems : 0;
  const avgMutuelleReimbursement =
  ...
  analyticsContent.innerHTML = `
                ${
                  data.mutuelle
                    ? `<div class="privacy-notice" style="background: #e8f5e8; color: #2d5016; border-left-color: #68d391; margin-bottom: 20px; text-align: center;">
                ...
                <div class="summary-cards">
                    <div class="summary-card">
                        ...
                        <div class="amount">${data.totalMutuelle.toFixed(
                          2
                        )}€</div>
                    </div>
                    <div class="summary-card">
                    ...
                    <div class="summary-card">
                        ...
                        <div class="amount" style="color: ${
                          totalDeficit > 0 ? "#38a169" : "#e53e3e"
                        }">${totalDeficit.toFixed(2)}€</div>
                    </div>
                    <div class="summary-card">
                        <h3>Bilan Mutuelle Annuel Moyen</h3>
                        <div class="amount" style="color: ${
                          avgDeficit > 0 ? "#38a169" : "#e53e3e"
                        }">${avgDeficit.toFixed(2)}€</div>
                    </div>
                    <div class="summary-card">
                    ...
                    <div class="summary-card">
                        ...
                        <div class="amount">${avgMutuelleReimbursement.toFixed(
                          2
                        )}€</div>
                    </div>
                    <div class="summary-card">
                        <h3>Remboursement Sécurité Sociale moyen</h3>
                        <div class="amount">${avgSecuReimbursement.toFixed(
                          2
                        )}€</div>
                    </div>
                    <div class="summary-card">
...
async function downloadAllChartsAsPDF() {
  try {
    ...
    yPos += 15;
    pdf.text("Remboursements de Mutuelle", pdfWidth / 2, yPos, {
      align: "center",
    });
    yPos += 20;
    ...
    if (analysisData.period) {
      pdf.text(`Période: ${analysisData.period}`, pdfWidth / 2, yPos, {
        align: "center",
      });
      yPos += 10;
    }
    ...
    if (analysisData.mutuelle) {
      pdf.text(`Mutuelle: ${analysisData.mutuelle}`, pdfWidth / 2, yPos, {
        align: "center",
      });
      yPos += 20;
    } else {
    ...
    const totalItems =
      analysisData.reimbursements.length + analysisData.pharmacyPayments.length;
    const avgReimbursement =
      totalItems > 0 ? analysisData.totalReimbursements / totalItems : 0;
    const avgMutuelleReimbursement =
      totalItems > 0 ? analysisData.totalMutuelle / totalItems : 0;
    const avgSecuReimbursement =
      totalItems > 0 ? analysisData.totalSecu / totalItems : 0;
    ...
    pdf.setFontSize(11);
    pdf.text(
      `Total remboursements Mutuelle: ${analysisData.totalMutuelle.toFixed(
        2
      )}€`,
      20,
      yPos
    ...
    yPos += 8;
    pdf.text(
      `Total remboursements Sécurité Sociale: ${analysisData.totalSecu.toFixed(
        2
      )}€`,
      20,
      yPos
    ...
    yPos += 8;
    pdf.text(
      `Bilan Mutuelle annuel moyen: ${avgDeficit.toFixed(2)}€`,
      20,
      yPos
    );
    yPos += 8;
    ...
    yPos += 8;
    pdf.text(
      `Remboursement Mutuelle moyen: ${avgMutuelleReimbursement.toFixed(2)}€`,
      20,
      yPos
    );
    yPos += 8;
    pdf.text(
      `Remboursement Sécurité Sociale moyen: ${avgSecuReimbursement.toFixed(
        2
      )}€`,
      20,
      yPos
      ...
      Object.keys(cotisationsData)
        ...
        .forEach((year, index) => {
          ...
          rowData.forEach((cell, i) => {
            pdf.text(cell, colPositions[i] + 2, yPos, {
              maxWidth: colWidths[i] - 4,
            });
          });
          yPos += 8;
